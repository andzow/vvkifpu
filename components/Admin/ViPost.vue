        ...
        <div class="post__container">
            <div class="post__position">
                <div class="post__wrapper">
                    ...
                    <Transition>
                        <img class="post__img" :src="imageUrl" alt="" v-if="imageUrl !== '' && useNews === 'Редактировать новость'">
                    </Transition>
                    <div class="post__circle" v-if="imageUrl !== ''" @click="clearFile">
                        <svg width="20" height="20" viewBox="0 0 41 43" fill="#542fe6"
                ...
                <input class="post__input" type="text" placeholder="Название" v-model="isName" @click="apple">
                <div class="post__block">
                    ...
                    <div class="post__other" :class="{ post__active: isOtherActive === true }">
                        <p class="post__item" :class="{ post__item_active: type === isType }"
                            v-for="(type, index) in isTypeArray" :key="index" @click="toType(type)">{{ type }}</p>
                    </div>
                </div>
            </div>
            <div class="post__description">
                <div class="post__array" v-for="(item, idx) in isInfo" :key="idx">
                    <div class="post__delay">
                        ...
                        <svg class="post__delete" width="20" height="20" viewBox="0 0 41 43" fill="#542fe6" @click="removeInfo" v-if="idx !== 0"
                            xmlns="http://www.w3.org/2000/svg">
                            <rect width="51.2508" height="7.32154" rx="3.66077"
                    ...
                    <div class="post__delay" v-for="(area, i) in item.info" :key="i">
                        <textarea class="post__textarea" 
                        placeholder="Содержание абзаца" v-model="item.info[i]"></textarea>
                        <svg class="post__delete_text" width="20" height="20" viewBox="0 0 41 43" fill="#542fe6" @click="removeParagraph(idx, i)" v-if="i !== 0"
                            xmlns="http://www.w3.org/2000/svg">
                            <rect width="51.2508" height="7.32154" rx="3.66077"
                                transform="matrix(0.699989 -0.714153 0.699989 0.714153 0 37.1289)" fill="#542fe6" />
                            <rect width="51.2508" height="7.32154" rx="3.66077"
                ...
                <div class="post__buttons">
                    <button class="post__button" @click="addInfo">Добавить заголовок</button>
                    <button class="post__button" @click="addParagraph">Добавить абзац</button>
                </div>
                <button class="post__button post__data" v-if="useNews === 'Создать новость'" @click="sendData">Создать новость</button>
                <button class="post__button update__data" v-if="useNews === 'Редактировать новость'" @click="updateData">Редактировать новость</button>
                <button class="post__button delete__data" v-if="useNews === 'Редактировать новость'" @click="deleteData">Удалить новость</button>
            </div>
        </div>
    </div>
</template>
